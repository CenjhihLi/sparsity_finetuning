...
from methods.finetuner_importance import HessianLabelImportance, TaylorLabelImportance, SecondTaylorLabelImportance
from methods.tp_MetaFinetuner import MetaFinetuner
from methods.tp_labelwise_importance import GroupTaylorLabelImportance, GroupHessianLabelImportance, GroupSecondTaylorLabelImportance
...
def main():
    ...
    if args.dependency:
        from methods.tp_labelwise_importance import LabelAggregators
        taylorImp = GroupTaylorLabelImportance
        hessianImp = GroupHessianLabelImportance
        SecondtaylorImp = GroupSecondTaylorLabelImportance
    else:
        from methods.finetuner_importance import LabelAggregators
        taylorImp = TaylorLabelImportance
        hessianImp = HessianLabelImportance
        SecondtaylorImp = SecondTaylorLabelImportance

    if args.label_aggregator in ['max', 'mean', 'GreaterQuantilesAvg', 'QuantilesAvg']:
        label_aggregator = LabelAggregators[args.label_aggregator]
    elif args.label_aggregator in ['Top5Avg', 'Top5MeanAvg']:
        label_aggregator = LabelAggregators[args.label_aggregator.replace("5", "k")](k = 5)
    ...
    if args.finetuning_type == 'random':
        ...
        #imp = GroupTaylorLabelImportance(method = torch.max, DEVICE = DEVICE)
        imp = taylorImp(label_aggregator = label_aggregator)
    elif args.finetuning_type == '2ndtaylor':
        imp = SecondtaylorImp(label_aggregator = label_aggregator)
        ...
        imp = hessianImp(label_aggregator = label_aggregator)
    else: raise NotImplementedError

    if args.dataset == 'tiny-imagenet':
        n_labels = 200
        n_label_data = 500
        batch_size_list = [10, 20, 25, 50, 100, 125, 250, 500]
    elif args.dataset == 'imagenet-1k':
        n_labels = 1000
        n_label_data = 1300
        batch_size_list = [10, 20, 25, 50, 100, 130, 260, 325, 650]
    ...
    if args.dataset == 'tiny-imagenet':
        ...
        labelwise_train_loader, labelwise_val_loader = label_order_tiny_imagenet(args)
        n_labels = 200
    elif args.dataset == 'imagenet-1k':
        ...
        labelwise_train_loader = torch.utils.data.DataLoader(dataset_train, batch_size = args.BATCH_SIZE, shuffle=False)
        val_loader = torch.utils.data.DataLoader(dataset_val, batch_size = args.BATCH_SIZE, shuffle=False)
        n_labels = 1000
    elif args.dataset == 'cifar100':
        from methods.data_loader import cifar100, label_order_cifar100
        print("Loading cifar100...")
        train_loader, val_loader = cifar100(args)
        labelwise_train_loader, labelwise_val_loader = label_order_cifar100(args)
        n_labels = 100
    ...
    for m in model.modules():
        if hasattr(m, 'weight'):
            if m.weight.requires_grad:
                size = tuple(list(m.weight.size()) + [n_labels])
                m.register_buffer('weight_grad_labelwise',
                    torch.zeros(size).to(m.weight.device))
        if hasattr(m, 'bias'):
            if m.bias is not None:
                if m.bias.requires_grad:
                    size = tuple(list(m.bias.size()) + [n_labels])
                    m.register_buffer('bias_grad_labelwise',
                        torch.zeros(size).to(m.bias.device))

    if isinstance(imp, (taylorImp, hessianImp, SecondtaylorImp)):
        model.zero_grad()
        print("Accumulating gradients for finetuning...")
        step = n_label_data/BATCH_SIZE
        for k, (imgs, lbls) in enumerate(labelwise_train_loader):
            label = int(k/step)
            imgs = imgs.to(DEVICE)
            lbls = lbls.to(DEVICE)
            output = model(imgs)
            if not isinstance(output, torch.Tensor):
            # assume that the model outputs a tuple of [outputs, outputs_kd] (This is for deit, deit returns (outputs, outputs_kd). )
                output, output_kd = output
                output = (output + output_kd)/2
            if isinstance(imp, (hessianImp, SecondtaylorImp)):
                loss = F.cross_entropy(output, lbls, reduction='none')
                for l in loss:
                    model.zero_grad()
                    l.backward(retain_graph=True)
                    imp.accumulate_grad(model, label, ignored_layers, n_label_data)
            elif isinstance(imp, taylorImp):
                loss = F.cross_entropy(output, lbls, reduction='sum')
                loss.backward()
                imp.accumulate_grad(model, label, ignored_layers, n_label_data)
    ...
    for m in model.modules():
        if hasattr(m, 'weight_grad_labelwise'):
            delattr(m, 'weight_grad_labelwise')
        if hasattr(m, 'bias_grad_labelwise'):
            delattr(m, 'bias_grad_labelwise')
    ...
    checkpoint_folder = os.path.join(os.path.join(checkpoint_folder, "ratio" + args.ratio_store), 
                                    ...
                                    ("_global" if args.global_finetuning else "") +\
                                    (("_labelwise_by_" + args.label_aggregator) if args.label_aggregator != "None" else "") )
    ...
    output_file = os.path.join(table_path, args.model + "_{}_".format(args.dataset) + ("inchannel_" if args.finetuning_channel == "in" else "") +\
                                    ...
                                    ("_global" if args.global_finetuning else "") +\
                                    (("_labelwise_by_" + args.label_aggregator) if args.label_aggregator != "None" else "") +\
                                    "_{}epochs.csv".format(args.EPOCHS))
